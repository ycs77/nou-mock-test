      ...
      >
        <label
          ...
          :class="{
            ...
            'ring-2 ring-success ring-offset-2 ring-offset-(--ui-bg-elevated)': answerMode && answer === option.value,
            'bg-primary text-white': answerMode && modelValue === option.value && modelValue === answer,
            'bg-neutral-200/50 dark:bg-neutral-700/50': answerMode && modelValue !== option.value,
            'bg-error text-white': answerMode && modelValue === option.value && modelValue !== answer,
          }"
        >
          <input
            v-model="modelValue"
...
import type { RadioBlock } from '#shared/types/exam'

interface ExamRadioProps extends RadioBlock {
  section: Block
  answerMode?: boolean
}
...
const modelValue = defineModel<RadioBlock['userAnswer']>({
  required: true,
  default: '',
})
