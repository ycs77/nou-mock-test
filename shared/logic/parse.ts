import type { Block, CheckboxBlock, FieldBlock, RadioBlock, SectionBlock, SectionType, TextareaBlock } from '../types/exam'
...
export function parseExam(content: string) {
  ...
  for (const line of lines) {
    ...
    if (sectionTitle === '是非題') {
      ...
      const section = blocks[blocks.length - 1] as SectionBlock
    ...
    // 解析選擇題
      ...
      if (line) {
        let answers: {
          number: string
          answer: Exclude<FieldBlock['answer'], undefined>
        }[] = []
        ...
        if (lineParts.length && lineParts.every(text => text.match(/^\d+\.[A-E]{1,3}$/))) {
          answers = line.match(/\d+\.[A-E]{1,3}/g)!.map(answerItem => {
            ...
            const number = m[1] || ''
            let answer: Exclude<FieldBlock['answer'], undefined> = (m[2] || '').trim()
            if (answer.length > 1) {
              answer = answer.split('')
            }
      ...
      let field: RadioBlock | CheckboxBlock = {
        type: 'radio',
        subject: line,
        options: [],
      }
    ...
    // 解析解釋名詞 or 簡答題 or 問答題
      ...
      if (line.match(/^(?:\d+\.|[一二三四五六七八九十]、)/)) {
        ...
        const section = blocks[blocks.length - 1] as SectionBlock
        let field = section.children[section.children.length - 1] as TextareaBlock | undefined
        if (!field) {
